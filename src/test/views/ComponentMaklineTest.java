package test.views; 

import org.junit.Test; 
import org.junit.Before; 
import org.junit.After; 

/** 
* ComponentMakline Tester. 
* 
* @author <Authors name> 
* @since <pre>Oct 27, 2015</pre> 
* @version 1.0 
*/ 
public class ComponentMaklineTest { 

@Before
public void before() throws Exception { 
} 

@After
public void after() throws Exception { 
} 

/** 
* 
* Method: getListCellRendererComponent(JList jList, Object value, int index, boolean isSelected, boolean cellHasFocus) 
* 
*/ 
@Test
public void testGetListCellRendererComponent() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getAccessibleName() 
* 
*/ 
@Test
public void testGetAccessibleName() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getAccessibleRole() 
* 
*/ 
@Test
public void testGetAccessibleRole() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getAccessibleIcon() 
* 
*/ 
@Test
public void testGetAccessibleIcon() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getAccessibleRelationSet() 
* 
*/ 
@Test
public void testGetAccessibleRelationSet() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getAccessibleText() 
* 
*/ 
@Test
public void testGetAccessibleText() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getIndexAtPoint(Point var1) 
* 
*/ 
@Test
public void testGetIndexAtPoint() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getCharacterBounds(int var1) 
* 
*/ 
@Test
public void testGetCharacterBounds() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getCharCount() 
* 
*/ 
@Test
public void testGetCharCount() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getCaretPosition() 
* 
*/ 
@Test
public void testGetCaretPosition() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getAtIndex(int var1, int var2) 
* 
*/ 
@Test
public void testGetAtIndex() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getAfterIndex(int var1, int var2) 
* 
*/ 
@Test
public void testGetAfterIndex() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getBeforeIndex(int var1, int var2) 
* 
*/ 
@Test
public void testGetBeforeIndex() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getCharacterAttribute(int var1) 
* 
*/ 
@Test
public void testGetCharacterAttribute() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getSelectionStart() 
* 
*/ 
@Test
public void testGetSelectionStart() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getSelectionEnd() 
* 
*/ 
@Test
public void testGetSelectionEnd() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getSelectedText() 
* 
*/ 
@Test
public void testGetSelectedText() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getAccessibleExtendedComponent() 
* 
*/ 
@Test
public void testGetAccessibleExtendedComponent() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getToolTipText() 
* 
*/ 
@Test
public void testGetToolTipText() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getTitledBorderText() 
* 
*/ 
@Test
public void testGetTitledBorderText() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getAccessibleKeyBinding() 
* 
*/ 
@Test
public void testGetAccessibleKeyBinding() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: validateObject() 
* 
*/ 
@Test
public void testValidateObject() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: addPropertyChangeListener(PropertyChangeListener var1) 
* 
*/ 
@Test
public void testAddPropertyChangeListenerPropertyChangeListener() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: removePropertyChangeListener(PropertyChangeListener var1) 
* 
*/ 
@Test
public void testRemovePropertyChangeListenerPropertyChangeListener() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getBorderTitle(Border var1) 
* 
*/ 
@Test
public void testGetBorderTitle() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getAccessibleDescription() 
* 
*/ 
@Test
public void testGetAccessibleDescription() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getAccessibleStateSet() 
* 
*/ 
@Test
public void testGetAccessibleStateSet() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getAccessibleChildrenCount() 
* 
*/ 
@Test
public void testGetAccessibleChildrenCount() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getAccessibleChild(int var1) 
* 
*/ 
@Test
public void testGetAccessibleChildI() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getKeyCodeArray() 
* 
*/ 
@Test
public void testGetKeyCodeArray() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: registerKeyPressed(int var0) 
* 
*/ 
@Test
public void testRegisterKeyPressed() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: registerKeyReleased(int var0) 
* 
*/ 
@Test
public void testRegisterKeyReleased() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: keyIsPressed(int var0) 
* 
*/ 
@Test
public void testKeyIsPressed() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: shouldProcess(KeyEvent var0) 
* 
*/ 
@Test
public void testShouldProcess() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: size() 
* 
*/ 
@Test
public void testSize() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: elementAt(int var1) 
* 
*/ 
@Test
public void testElementAt() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: addElement(int var1) 
* 
*/ 
@Test
public void testAddElement() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: setElementAt(int var1, int var2) 
* 
*/ 
@Test
public void testSetElementAt() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getValue(String var1) 
* 
*/ 
@Test
public void testGetValue() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: isEnabled() 
* 
*/ 
@Test
public void testIsEnabled() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: actionPerformed(ActionEvent var1) 
* 
*/ 
@Test
public void testActionPerformed() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: putValue(String var1, Object var2) 
* 
*/ 
@Test
public void testPutValue() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: setEnabled(boolean var1) 
* 
*/ 
@Test
public void testSetEnabledB() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getAccessibleAt(Point var1) 
* 
*/ 
@Test
public void testGetAccessibleAtPoint() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: run() 
* 
*/ 
@Test
public void testRun() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: accept(Component var1) 
* 
*/ 
@Test
public void testAcceptComponent() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getAccessibleParent() 
* 
*/ 
@Test
public void testGetAccessibleParent() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getAccessibleIndexInParent() 
* 
*/ 
@Test
public void testGetAccessibleIndexInParent() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getLocale() 
* 
*/ 
@Test
public void testGetLocale() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getAccessibleComponent() 
* 
*/ 
@Test
public void testGetAccessibleComponent() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getBackground() 
* 
*/ 
@Test
public void testGetBackground() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: setBackground(Color var1) 
* 
*/ 
@Test
public void testSetBackground() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getForeground() 
* 
*/ 
@Test
public void testGetForeground() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: setForeground(Color var1) 
* 
*/ 
@Test
public void testSetForeground() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getCursor() 
* 
*/ 
@Test
public void testGetCursor() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: setCursor(Cursor var1) 
* 
*/ 
@Test
public void testSetCursor() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getFont() 
* 
*/ 
@Test
public void testGetFont() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: setFont(Font var1) 
* 
*/ 
@Test
public void testSetFont() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getFontMetrics(Font var1) 
* 
*/ 
@Test
public void testGetFontMetrics() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: isVisible() 
* 
*/ 
@Test
public void testIsVisible() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: setVisible(boolean var1) 
* 
*/ 
@Test
public void testSetVisible() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: isShowing() 
* 
*/ 
@Test
public void testIsShowing() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: contains(Point var1) 
* 
*/ 
@Test
public void testContains() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getLocationOnScreen() 
* 
*/ 
@Test
public void testGetLocationOnScreen() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getLocation() 
* 
*/ 
@Test
public void testGetLocation() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: setLocation(Point var1) 
* 
*/ 
@Test
public void testSetLocation() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getBounds() 
* 
*/ 
@Test
public void testGetBounds() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: setBounds(Rectangle var1) 
* 
*/ 
@Test
public void testSetBounds() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getSize() 
* 
*/ 
@Test
public void testGetSize() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: setSize(Dimension var1) 
* 
*/ 
@Test
public void testSetSize() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: isFocusTraversable() 
* 
*/ 
@Test
public void testIsFocusTraversable() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: requestFocus() 
* 
*/ 
@Test
public void testRequestFocus() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: addFocusListener(FocusListener var1) 
* 
*/ 
@Test
public void testAddFocusListener() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: removeFocusListener(FocusListener var1) 
* 
*/ 
@Test
public void testRemoveFocusListener() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: acceptRequestFocus(Component var1, Component var2, boolean var3, boolean var4, Cause var5) 
* 
*/ 
@Test
public void testAcceptRequestFocus() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getCapabilities() 
* 
*/ 
@Test
public void testGetCapabilities() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getDrawGraphics() 
* 
*/ 
@Test
public void testGetDrawGraphics() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: contentsLost() 
* 
*/ 
@Test
public void testContentsLost() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: contentsRestored() 
* 
*/ 
@Test
public void testContentsRestored() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: show() 
* 
*/ 
@Test
public void testShow() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: show(int var1, int var2, int var3, int var4) 
* 
*/ 
@Test
public void testShowForII1I2I3() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: showIfNotLost(int var1, int var2, int var3, int var4) 
* 
*/ 
@Test
public void testShowIfNotLostForII1I2I3() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: dispose() 
* 
*/ 
@Test
public void testDispose() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: createBackBuffers(int var1) 
* 
*/ 
@Test
public void testCreateBackBuffers() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: getBackBuffer() 
* 
*/ 
@Test
public void testGetBackBuffer() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: showSubRegion(int var1, int var2, int var3, int var4) 
* 
*/ 
@Test
public void testShowSubRegionForII1I2I3() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: revalidate() 
* 
*/ 
@Test
public void testRevalidate() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: revalidate(boolean var1) 
* 
*/ 
@Test
public void testRevalidateB() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: createBuffers(int var1, BufferCapabilities var2) 
* 
*/ 
@Test
public void testCreateBuffers() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: flip(FlipContents var1) 
* 
*/ 
@Test
public void testFlip() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: flipSubRegion(int var1, int var2, int var3, int var4, FlipContents var5) 
* 
*/ 
@Test
public void testFlipSubRegion() throws Exception { 
//TODO: Test goes here... 
} 

/** 
* 
* Method: destroyBuffers() 
* 
*/ 
@Test
public void testDestroyBuffers() throws Exception { 
//TODO: Test goes here... 
} 


/** 
* 
* Method: getText(int var1, int var2) 
* 
*/ 
@Test
public void testGetText() throws Exception { 
//TODO: Test goes here... 
/* 
try { 
   Method method = ComponentMakline.getClass().getMethod("getText", int.class, int.class); 
   method.setAccessible(true); 
   method.invoke(<Object>, <Parameters>); 
} catch(NoSuchMethodException e) { 
} catch(IllegalAccessException e) { 
} catch(InvocationTargetException e) { 
} 
*/ 
} 

/** 
* 
* Method: getTextRectangle() 
* 
*/ 
@Test
public void testGetTextRectangle() throws Exception { 
//TODO: Test goes here... 
/* 
try { 
   Method method = ComponentMakline.getClass().getMethod("getTextRectangle"); 
   method.setAccessible(true); 
   method.invoke(<Object>, <Parameters>); 
} catch(NoSuchMethodException e) { 
} catch(IllegalAccessException e) { 
} catch(InvocationTargetException e) { 
} 
*/ 
} 

/** 
* 
* Method: registerComponent(JComponent var1) 
* 
*/ 
@Test
public void testRegisterComponent() throws Exception { 
//TODO: Test goes here... 
/* 
try { 
   Method method = ComponentMakline.getClass().getMethod("registerComponent", JComponent.class); 
   method.setAccessible(true); 
   method.invoke(<Object>, <Parameters>); 
} catch(NoSuchMethodException e) { 
} catch(IllegalAccessException e) { 
} catch(InvocationTargetException e) { 
} 
*/ 
} 

/** 
* 
* Method: updateInternalBuffers() 
* 
*/ 
@Test
public void testUpdateInternalBuffers() throws Exception { 
//TODO: Test goes here... 
/* 
try { 
   Method method = ComponentMakline.getClass().getMethod("updateInternalBuffers"); 
   method.setAccessible(true); 
   method.invoke(<Object>, <Parameters>); 
} catch(NoSuchMethodException e) { 
} catch(IllegalAccessException e) { 
} catch(InvocationTargetException e) { 
} 
*/ 
} 

} 
